...
export function CourseSegmentationSection() {
  return (
    <SectionWrapper id="course-segmentation" className="bg-slate-50 border-b border-slate-200/80 transition-colors duration-300">
      {/* Header */}
      <div className="text-center max-w-3xl mx-auto space-y-3 mb-12">
        <h2 className="text-3xl md:text-4xl font-bold text-slate-900">
          Tailored <span className="text-primary-600">Course Segmentation</span>
        </h2>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-slate-900">{segment.title}</h3>
              ...
              <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-100">
                {segment.tags.map((tag) => (
                  <Badge key={tag} variant="neutral" className="bg-slate-100 text-slate-700">
                    {tag}
                  </Badge>
                ))}
              </div>
